...
def addList():
    ...
    vendor.get(),model.get(),typeClass.get(),
    regNumber.get(),engNumber.get(),mileage.get()))
...
def filterList():
    global listOfVehicles
    if(owner.get()!="" or vendor.get()!="" or model.get()!="" or typeClass.get()!=""
    or mileage.get()!=""):
    ...
    if(owner.get()!=""):
        ...
        for i in listOfVehicles:
            if i['ownerName']==filterKey:
                ...
                (i['ownerName'],i['vendor'],i['model'],i['type'],i['registrationNumber'],
...
def loadFile():
    ...
    for i in listOfVehicles:
        ...
        i['registrationNumber'],i['engineNumber'],
        i['mileage']))
...
def sortMileage():
    ...
    for i in sortedList:
        treeList.insert(parent='', index='end', text="", values=(i['ownerName'],
        i['vendor'],i['model'], i['type'],
    i['registrationNumber'],i['engineNumber'],
    i['mileage']))
...
button1=Button(window,width=10,text="Load Pickle file",bg='#0000FF',command=loadFile)
button1.grid(row=2,column=4)

button8=Button(window,width=10,text="Filter all",bg='#0000FF',command=filterList)
button8.grid(row=2,column=5)

#second section
#row - 0
button2=Button(window,width=10,text="Add details",bg='#0000FF',command=addList)
button2.grid(row=0,column=4)

#row - 1
button4=Button(window,width=10,text="Delete vehicle details",bg='#0000FF',command=delete)
button4.grid(row=1,column=4)

button5=Button(window,width=10,text="Sort by Mileage",bg='#0000FF',command=sortMileage)
button5.grid(row=1,column=5)

#row - 2
# button6=Button(window,width=10,text="Filter",bg='#0000FF')
# button6.grid(row=2,column=4)

button7=Button(window,width=10,text="Create Pickle",bg='#0000FF',command=createPickle)
button7.grid(row=0,column=5)


style = Style()
style.theme_use("alt")
style.configure("Treeview",bg="black",foreground='green')
style.map('Treeview',background=[('selected','#000000')])
treeList = Treeview(columns=("Owner","Vendor","Model","Type","Reg Number","Eng Number",
"Mileage"),show='headings')
