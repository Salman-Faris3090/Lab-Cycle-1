...
text = font.render('RESET', True,BG_COLOR,TILE_COLOUR)
...
def display_message(content):
    ...
    screen.blit(end_text, ((WIDTH - end_text.get_width())// 2, 
    (WIDTH - end_text.get_height()) // 2))
	...
	for row in range(BOARD_ROWS):
		for col in range(BOARD_COLS):
			if board[row][col] == 1:
				...
				pygame.draw.line( screen, CROSS_COLOR, (col * SQUARE_SIZE + SPACE, row *
				SQUARE_SIZE + SPACE), (col * SQUARE_SIZE +
				SQUARE_SIZE - SPACE, row * SQUARE_SIZE +
				SQUARE_SIZE - SPACE), CROSS_WIDTH )
